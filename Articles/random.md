`random`是一个关键字，它允许我们将“随机性”添加到我们的模型中。当我们为`random`提供一个数字时，它会返回一个从 0 到 N-1 的随机数。想象一下，每次运行函数`random N` ，您都在掷一个有 N 个面的骰子。例如，如果我们想创建一场森林火灾，我们将每个格子指定为一棵树（ `pcolor = green` ）或地面（ `pcolor = brown` ），但树密度为 70%，我们将编写以下代码：
